                    ...
                    <form method="POST" action="{{ route('comments.store') }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6 mt-2">
                                <input type="checkbox" class="@error('subscribed') is-invalid @enderror" id="subscribed" name="subscribed" value="1" {{ old('subscribed') ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6">
                                <select id="article" class="form-control @error('article_id') is-invalid @enderror" name="article_id" required>
                                    @foreach($articles as $article)
                                        <option value="{{ $article->id }}" {{ old('article_id') == $article->id ? 'selected' : '' }}>{{ $article->title }}</option>
                                    @endforeach
                                </select>
